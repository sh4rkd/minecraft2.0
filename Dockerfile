...
COPY ["run.sh", "run.bat", "forge-1.20.1-47.3.27-installer.jar", "./"]
...
RUN java -jar forge-1.20.1-47.3.27-installer.jar --installServer && \
    if [ -f "libraries/net/minecraftforge/forge/1.20.1-47.3.27/forge-1.20.1-47.3.27-server.jar" ]; then \
        cp libraries/net/minecraftforge/forge/1.20.1-47.3.27/forge-1.20.1-47.3.27-server.jar server.jar && \
        echo "Server jar copied from libraries directory"; \
    elif [ -f "forge-1.20.1-47.3.27-server.jar" ]; then \
        cp forge-1.20.1-47.3.27-server.jar server.jar && \
        ...
        ls -la libraries/net/minecraftforge/forge/1.20.1-47.3.27/ && \
        exit 1; \
    fi && \
    rm forge-1.20.1-47.3.27-installer.jar && \
    # Verificar que el server.jar existe
    ls -la server.jar

# Configurar script de inicio con parámetros optimizados
RUN chmod +x run.sh run.bat
